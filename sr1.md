...
def summ(a1, d, n):
    ...
    Проверяет, если номер последнего члена не равен одному, тогда высчитывает сумму первых n-членов арифметической 
    прогрессии по формуле, иначе эта сумма равна первому члену прогрессии 
    """
    if n != 1: 
        sn = n * (2 * a1 + (n - 1) * d) / 2
    else:    
